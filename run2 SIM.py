...
# popt_lst_SIM =  [np.array([ 4*1.        ,  51.12838922,   0.41547372,  -0.20249376, -7.0*2,   8.67477311]),
#                  np.array([ 7*1.        ,  51.12838922,   0.41547372,  -0.20249376, -7.0*2,   8.67477311]),
#                  np.array([ 7*1.        ,  51.12838922,   0.41547372,  -0.20249376, -7.0*2,   8.67477311]),
#                  np.array([ 7*1.        ,  51.12838922,   0.41547372,  -0.20249376, -7.0*2,   8.67477311])]
popt_lst_SIM =  [np.array([  5.        , 70.84003388,  0.22195401, -0.36003422, -4.89686332, -9.36673184]),
                  np.array([ 5.        , 70.84003388,  3.22195401, -0.36003422, -4.89686332, -9.36673184]),
                  np.array([ 5.        , 70.84003388,  0.22195401, -0.36003422, -4.89686332, -9.36673184]),
                  np.array([ 5.        , 70.84003388,  0.22195401, -0.36003422, -4.89686332, -9.36673184])]
# popt_lst_SIM =  [np.array([ 0.        , 100,  -5,  -0.5, -7,   0]),
#                  np.array([ 0.        , 100,  -5,  -0.5, -7,   0]),
#                  np.array([ 0.        , 100,  -5,  -0.5, -7,   0]),
#                  np.array([ 0.        , 100,  -5,  -0.5, -7,   0])]
# popt_lst_SIM =  [np.array([ 0.        , 125.47013621,  -4.49317685,  -0.48064665, -5.01969974,   1.51777175]),
#                   np.array([ 0.        , 70.84003388,  0.22195401, -0.36003422, -4.89686332, -9.36673184]),
#                   np.array([ 0.        , 108.53425759,   1.8144134 ,  -0.28766857, -5.30669516, -13.7524413 ]),
#                   np.array([ 0.        ,  51.12838922,   4.41547372,  -0.20249376, -14.79186002,   8.67477311])]
...
for ind in inds:
    ...
    PSF_curr = 1.0#DIFF_df.iloc[3]['FWHM_ang'].arcsec
    shape = (len(x),1)
    input_arr = np.concatenate((x.reshape(shape),y.reshape(shape),mjd_curr*np.ones(shape),PSF_curr*np.ones(shape)),axis=1)
    input_arr_s.append(input_arr)
    
    # z=FP_df_lst[0].iloc[ind]['FluxProfile_ADU']
    aug_noise_sigma = 5
    noise = np.random.normal(loc=0,scale=aug_noise_sigma,size=x.shape)
    z = ConvLC_2Dpolyphase(input_arr, *popt_lst_SIM[ind]) + noise
...
# def SN_f(phase):
#     d_wid = 1
#     flux = 100.0*(np.abs(phase)<(d_wid/2))
#     return flux
...
bnd = ([-0.0,     1e1,            -1e2,   -50.0*np.pi/180,  -4e1, -5e1 ],
        [100.0,    1e3,             1e2,    50.0*np.pi/180,   -1.3e-1, 5e1])
...
for i in np.arange(len(input_arr_s)):
    ...
    mjd = inp_arr[:,2][0]
    popt, pcov = curve_fit(ConvLC_2Dpolyphase,inp_arr,z,bounds=bnd)#,absolute_sigma=True,sigma=np.ones(inp_arr[:,0].shape)*aug_noise_sigma)
