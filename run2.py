...
Orgs=SkyCoord([[12.39251174, 58.76486208]],frame='fk5',unit=(u.deg, u.deg)) # PA = 180
...
y_cutoff = [[-1.5,3]]*4#[[-5,7.5],   [-7.5,10],  [-7.5,10]]
...
def ConvLC_2Dpolyphase(input_array, p01,p02,p04,p05,p06,add_const):
    ...
    eff_phs_wid = p01#np.hypot(p01,phase_grad*PSFs[inds_umjd[0][0]])
...
bnd = ([0.0,     1e1,            -1e3,   -50.0*np.pi/180,  -4e1, -5e1 ],
        [200.0,    1e3,             1e3,    50.0*np.pi/180,   -1.3e1, 5e1])
...
for i in np.arange(len(input_arr_s)):
    ...
    mjd = inp_arr[:,2][0]
    popt, pcov = curve_fit(ConvLC_2Dpolyphase,inp_arr,z,bounds=bnd)
    ...
    ax.scatter(x,y,ConvLC_2Dpolyphase(inp_arr,*popt),cmap='jet',vmax=F_SCALE,vmin=-F_SCALE,c='b',s=1)
    ax.scatter(x,y,z,cmap='jet',vmax=F_SCALE,vmin=-F_SCALE,c='r',s=1)
    plt.xlabel('x [arcsec]')
    plt.ylabel('y [arcsec]')
    plt.title(mjd)
    axisEqual3D(ax)
...
for fig in tqdm(figures):
    ...
    #                inc = inc
                span=clim[1]-clim[0]
                im.set_clim(clim[0],clim[1]+1.0*span)
            fig.canvas.draw()
# %%
plt.figure()
