...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 16 17:26:41 2021
...
# phs_grd_lst = [13.894]*len(filenames)
...
for slit_ind in slit_inds:
    ...
    for i in inds:
        ...
        if estimate_flux_bias:
            ...
            z = z-bias_est
            paramss['add_const'].value = 0.0*bias_est # estimate flux bias
        ...
        ax.plot(x_sorted,LEprof_model(x_sorted,*popt,ignore_dust=True,ignore_PSF=True)-0.0*result.params['add_const'],c='k',linewidth=1,label='light curve')
        # ax.plot(x_sorted,LEprof_model(x_sorted,*popt,ignore_dust=True),c='b',linewidth=0.5,label='PSF only')
        ax.plot(x_sorted*x_scaling,LEprof_model(x_sorted,*popt)+const_offs-0.0*result.params['add_const'],c='r',linewidth=2,label='fit (PSF+dust)')
        ...
        ax.scatter(x*x_scaling,z+const_offs-0.0*result.params['add_const'],s=0.5, label='data')
...
plt.scatter(mjds.flatten(),peaks.flatten(),c='b',label='Peaks')

plt.plot(mjds.flatten(),intercept+slope*mjds.flatten(),'--',c='k',label='Regression: '+'{:.1f}'.format(np.abs(365*slope))+' [arcsec/year]')
...
for pg,mjd,pk in zip(phs_grads,mjds.flatten(),peaks.flatten()):
    pnts_y = pk + np.array([-1,1])
    pnts_x = mjd - np.array([-1,1])*pg
    if i==0:
        lbl = 'apparent motion from fitting'
    else: lbl=''
    plt.plot(pnts_x,pnts_y,c='r',label=lbl)
    print('local_phs_grad: '+'{:.1f}'.format(np.abs(365/pg))+' [arcsec/year]')
    i=i+1
plt.legend()
