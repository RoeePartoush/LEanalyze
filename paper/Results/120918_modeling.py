...
# == Time Series #3 ==
Orgs=SkyCoord([(12.362678, 58.7583544)],frame='fk5',unit=(u.deg, u.deg))
Orgs=SkyCoord([Orgs[0].directional_offset_by(Angle(169,u.deg),Angle(2,u.arcsec))])
PA = Angle([Angle(-11,'deg') for Org in Orgs])+Angle(180,u.deg)
Ln = Angle([20  for Org in Orgs],u.arcsec)
Wd = Angle([10  for Org in Orgs],u.arcsec)


y_cutoff = np.sort(-np.array([[-5,0],[0,5],[-2,5]])).tolist()
x_cutoff = np.sort(-np.array([[-10,20],[-10,10],[-10,2]])).tolist()


root = '/Users/roeepartoush/Documents/Research/Data/swarp_test/tycA1/'
filenames = ['20120626/tyc4519.VR.20120624.24652_stch_5.sw.fits',
             'KECK/tyc4419_1.R.r120918_0182_4.hdrfix_CORRECTED_DEG_MJD.sw._NOT_REALLY_coadd.fits',
             '20130109/tyc4519.VR.20130109t.1692977_stch_5.sw.fits']

img_titles = ['KP, VR, 20120626',
              'KECK, R, 20120918',
              'KP, VR, 20130109']
...
LEplots.match_zoom_wcs(axs,w_s,slitFPdf.iloc[0]['Orig'],slitFPdf.iloc[0]['Length']*1.6,slitFPdf.iloc[0]['Length']*1.5)
...
for slit_ind in slit_inds:
    ...
    for i in inds:
        ...
        ax2.set_xlabel('x [days]')#r"Modified x-axis: $1/(1+X)$")
