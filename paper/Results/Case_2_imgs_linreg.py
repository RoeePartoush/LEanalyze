...
# -*- coding: utf-8 -*-
"""
Created on Fri May 13 16:19:53 2022
...
x_cutoff = np.sort(-np.array([[-19,-10],[-11,-2.5],[3,-3],  [0,12]])).tolist()
...
print('\n\n=== Plotting Images... ===')
plt.close('all')
figures=[plt.figure()]*4#[plt.figure() for i in DIFF_df.index]
fig_fig = figures[0]
# figures = [figures[0]]*8
...
for i in DIFF_df.index:
    axs.append(plt.subplot2grid((2,len(DIFF_df)),(0,i),rowspan=1,projection=DIFF_df.WCS_w[i],fig=figures[0]))

CLs = np.zeros((len(DIFF_df),3,2))
prev_known_peaks = [-22.92, -27.14, -28.93]
for i in np.arange(len(DIFF_df)):
    CLs[i,0,0], CLs[i,1,0], CLs[i,2,0] = np.array([1,1,1])*prev_known_peaks[i]
    CLs[i,0,1], CLs[i,1,1], CLs[i,2,1] = np.array([-1,0,1])*0.5*slitFPdf.WIDTH[0].arcsec

axs = LEplots.imshows(DIFF_df,plot_Fprofile=True, crest_lines = CLs, img_axs=axs, profDF=slitFPdf, prof_sampDF_lst=None, prof_crop=(x_cutoff,y_cutoff), popts=popt_lst, g_cl_arg=coord_list, FullScreen=False, figs=figures)
# axs = LEplots.imshows(DIFF_df,plot_Fprofile=True, img_axs=axs, profDF=slitFPdf, prof_sampDF_lst=FP_df_lst, prof_crop=(x_cutoff,y_cutoff), popts=popt_lst, g_cl_arg=coord_list, FullScreen=False, figs=figures)


w_s = DIFF_df['WCS_w'].to_list()
LEplots.match_zoom_wcs(axs,w_s,slitFPdf.iloc[0]['Orig'].directional_offset_by(PA[0],Angle(-25,u.arcsec)),slitFPdf.iloc[0]['Length']*0.20,slitFPdf.iloc[0]['Length']*0.2)
...
for i,ax in enumerate(axs):
    ax.title.set_text(img_titles[i])
...
plt.sca(plt.subplot2grid((2,len(DIFF_df)),(1,0),rowspan=1,colspan=len(DIFF_df),fig=fig_fig))
...
plt.scatter(mjds.flatten()-MJDoffset,peaks.flatten(),linestyle='None',color='b',label='Peaks')
...
plt.plot(mjds.flatten()-MJDoffset,intercept+slope*(mjds.flatten()-MJDoffset),'--',color='k',label='Regression: '+'{:.1f}'.format(np.abs(365*slope))+' [arcsec/year]')
plt.legend()
...
for i,(x,y) in enumerate(zip(mjds.flatten()-MJDoffset,peaks.flatten())):
    plt.gca().text(x, y-1*np.sign(y+25), img_titles[i], fontsize=12,verticalalignment='top', bbox=props)
