...
Orgs=SkyCoord([Orgs[0].directional_offset_by(Angle(PA_DEF,u.deg),Angle(6.5,u.arcsec)).directional_offset_by(Angle(PA_DEF+90,u.deg),Angle(ofs,u.arcsec)) for ofs in np.linspace(-2.5,2.5,3)+1])
...
x_cutoff = np.sort(-np.array([[1,-1]])*8.0).tolist()
...
for slit_ind in slit_inds:
    ...
    for i in inds:
        ...
        const_offs_scale = 40
        const_offs = const_offs_scale*(len(slit_inds)-1-slit_ind)
        ...
        clean_LC = peak_flux*SN_f((x_arcsec-peak_loc)*grad)/SN_f(0)
        ...
        ax_wf_days.plot(x_days,const_offs+clean_LC,'--',c='k',linewidth=1,label='')
        ax_wf_days.plot(x_days,const_offs+eff_LC,linewidth=2,label='eff. light curve')
        ax_wf_days.plot(x_days,const_offs+eff_LC*0,':',c='k',linewidth=1,label='')
...
ax_wf_days.plot(x_days,(const_offs/const_offs_scale-1)*const_offs_scale+total_eff_LC,c='b',linewidth=3,label='total eff. light curve')
ax_wf_days.plot(x_days,(const_offs/const_offs_scale-1)*const_offs_scale+clean_LC,'--',c='k',linewidth=1,label='')
ax_wf_days.plot(x_days,(const_offs/const_offs_scale-1)*const_offs_scale+eff_LC*0,':',c='k',linewidth=1,label='')
plt.legend()
