...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 25 12:07:12 2021
...
phs_grd_lst = [13.8]*len(filenames)
...
LEplots.match_zoom_wcs(axs,w_s,slitFPdf.iloc[0]['Orig'].directional_offset_by(slitFPdf.iloc[0]['PA'],Angle(-25,u.arcsec)),slitFPdf.iloc[0]['Length']*0.35,slitFPdf.iloc[0]['Length']*0.3)
...
for slit_ind in slit_inds:
    ...
    for i in inds:
        ...
        if estimate_flux_bias:
            ...
            args = (x-x.min())<(x_span/10)
            bias_est = np.mean(z[args])
            z = z-bias_est
            paramss['p1_add_const'].value = 0*bias_est # estimate flux bias
        ...
        paramss.add('p1_p2_pldiff',value=0.5, min=0.01, max=10.0)
        paramss['p2_peak_loc'].expr = 'p1_peak_loc + p1_p2_pldiff'
        ...
        ax.plot(x_sorted*x_scaling,LE_model.eval(input_array=x_sorted,params=result.params)+const_offs-0*result.params['p1_add_const'],c='r',linewidth=2,label='fit (PSF+dust)')
        ...
        # ax.plot(x_arcsec*x_scaling,LE_model.eval(input_array=x_arcsec,params=params1)+const_offs-0*result.params['p1_add_const'],c=p12_colors[0],linewidth=1.5,label='fit peak 1')
        # ax.plot(x_arcsec*x_scaling,LE_model.eval(input_array=x_arcsec,params=params2)+const_offs-0*result.params['p1_add_const'],c=p12_colors[1],linewidth=1.5,label='fit peak 2')
        
        params1 = deepcopy(result.params)
        for pnm in [pnm for pnm in LE_model.param_names if pnm.startswith('p2')]:
            _ = params1.pop(pnm)
        params2 = deepcopy(result.params)
        for pnm in [pnm for pnm in LE_model.param_names if pnm.startswith('p1')]:
            _ = params2.pop(pnm)
        ax.plot(x_arcsec*x_scaling,LE_model1.eval(input_array=x_arcsec,params=params1)+const_offs-0*result.params['p1_add_const'],'--',c=p12_colors[0],linewidth=1.5,label='fit peak 1')
        ax.plot(x_arcsec*x_scaling,LE_model2.eval(input_array=x_arcsec,params=params2)+const_offs-0*result.params['p1_add_const'],'--',c=p12_colors[1],linewidth=1.5,label='fit peak 2')
        plt.legend()
        ...
        ax.scatter(x*x_scaling,z+const_offs-0*result.params['p1_add_const'],s=0.5, label='data')
        ...
        phs_grads_12 = [result.params['p1_phs_grad'].value, result.params['p2_phs_grad'].value]
        peak_locs_12 = [result.params['p1_peak_loc'].value, result.params['p2_peak_loc'].value]
        peak_fluxs_12 = [result.params['p1_peak_flux'].value, result.params['p2_peak_flux'].value]
        sig_dusts_12 = [result.params['p1_sig_dust'].value, result.params['p2_sig_dust'].value]
        ...
        ax.fill_between(x_arcsec,0,rect_win*result.params['p1_peak_flux'].value,color='r',alpha=0.3,label='slitlet aperture')
        plt.legend()
