...
# x_cutoff = np.sort(-np.array([[3,-3]]*4)).tolist()


# == Single Image #1 ==
Orgs=SkyCoord([(12.37159974, 58.75964257)],frame='fk5',unit=(u.deg, u.deg))
PA = Angle([155],'deg')+Angle(180,u.deg)
Ln = Angle([30]*1,u.arcsec)
Wd = Angle([2],u.arcsec)
x_cutoff = [[-8,15]]

Orgs = SkyCoord([Orgs[0].directional_offset_by(PA[0]-Angle(0,u.deg),Angle(0.5,u.arcsec))])

filenames = ['KECK/tyc4419_1_R_LRIS_131228_coadd.fits']

root = '/Users/roeepartoush/Documents/Research/Data/swarp_test/tycA1/'
# filenames = ['20120118/tyc4419.VR.20120118.216797_stch_1.sw.fits',#'tyc4419_tyc4519_20120118_coadd.fits',
#     '20120626/tyc4519.VR.20120624.24652_stch_5.sw.fits',#'tyc4419_tyc4519_20120626_coadd.fits',
#         'KECK/tyc4419_1.R.r120918_0182_4.hdrfix_CORRECTED_DEG_MJD.sw._NOT_REALLY_coadd.fits',
#     '20130109/tyc4519.VR.20130109t.1692977_stch_5.sw.fits']#'tyc4419_tyc4519_20130109_coadd.fits']
img_titles = ['KP, VR, 20120118',
              'KP, VR, 20120626',
              'KECK, R, 20120918',
              'KP, VR, 20130109']

# phs_grd_lst = [15.97]*len(filenames)
...
LEplots.match_zoom_wcs(axs,w_s,slitFPdf.iloc[0]['Orig'],slitFPdf.iloc[0]['Length']*2.20,slitFPdf.iloc[0]['Length']*8.2)
...
for slit_ind in slit_inds:
    ...
    for i in inds:
        ...
        # ax.legend()
        def tick_function(x_arc, dtype='str'):
            x_d = (x_arc-peak_loc)*grad
            if dtype=='str':
                return ["%.0f" % z for z in x_d]
            else:
                return x_d
        
        def inv_tick_function(x_d, dtype='str'):
            x_arc = peak_loc + (x_d/grad)
            if dtype=='str':
                return ["%.0f" % z for z in x_arc]
            else:
                return x_arc
        ...
        ax1 = fig.get_axes()[1]
        ax2 = ax1.twiny()
        x_d_lim1 = np.round(tick_function(ax1.get_xlim()[0],dtype='num')+0.5*10**(1),-1)
        print('KKKKKKK!!! 2022')
        print(np.round(tick_function(ax1.get_xlim()[0],dtype='num')+0.5*10**(1),-1))
        print(x_d_lim1)
        x_d_lim2 = np.round(tick_function(ax1.get_xlim()[1],dtype='num')-0.5*10**(1),-1)
        new_tick_locations = inv_tick_function(np.linspace(x_d_lim1,x_d_lim2,int((x_d_lim2-x_d_lim1)/10+1)),dtype='num')
        new_tick_locations = new_tick_locations[1::2]
        # new_tick_locations = ax1.xaxis.get_ticklocs()#np.array([.2, .5, .9])
        
        ax2.set_xlim(ax1.get_xlim())
        ax2.set_xticks(new_tick_locations)
        ax2.set_xticklabels(tick_function(new_tick_locations))
        ax2.set_xlabel('ρ [days]')#r"Modified x-axis: $1/(1+X)$")
